...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('modalAddToCartBtn').addEventListener('click', async () => {
    if (selectedBookForModal) {
      await addToCart(selectedBookForModal._id || selectedBookForModal.id);
...
async function renderizarVistaCarritoCompleta() {
  ...
  try {
    ...
    cartItems.forEach(item => {
      const bookData = allBooksLocal.find(b => (b._id === item.book_id || b.id == item.book_id)) || item;
      ...
      itemRow.innerHTML = `
        ...
        <div style="text-align:right;">
          ...
          <button onclick="eliminarDelCarritoCompleto('${item._id || item.id}')" style="background:none; border:none; color:#e53e3e; cursor:pointer;">Eliminar</button>
        </div>
      `;
...
async function procesarCompraFinal() {
  ...
  try {
    ...
    for (const item of cartItems) {
      ...
      await fetch(`${API_URL}/cart/${item._id || item.id}`, { method: 'DELETE' });
    }
...
async function loadUserPurchases() {
  ...
  try {
    ...
    userPaidBookIds = sales.filter(s => s.user_email === userEmail).map(s => s.book_id);
...
function renderBooks(booksList) {
  ...
  booksList.forEach(book => {
    ...
    const hasAccess = userRole === 'admin' || userPaidBookIds.includes(bookId);
    ...
    card.innerHTML = `
      ...
      <div>
        ...
        <button class="btn-action" style="background-color:${hasAccess ? '#00f5d4':'#2d3748'}; color:${hasAccess ? 'black':'white'}" onclick="handleBookAccess('${bookId}')">
...
function handleBookAccess(bookId) {
  const book = allBooksLocal.find(b => (b._id === bookId || b.id == bookId));
  const userRole = localStorage.getItem('userRole');
  const hasAccess = userRole === 'admin' || userPaidBookIds.includes(bookId);
